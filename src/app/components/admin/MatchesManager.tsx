  ...
  return (
    <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, league: e.target.value as 'futsal' | 'campo', homeTeamId: '', awayTeamId: '' })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">Selecione um time</option>
                  {leagueTeams.filter(t => t.id !== formData.homeTeamId).map(team => (
                    <option key={team.id} value={team.id}>{team.name}</option>
                  ))}
                </select>
              </div>
      ...
      <div className="space-y-6">
        {['futsal', 'campo'].map((league) => (
          <div key={league}>
            <h3 className="text-sm text-gray-600 mb-3">
              Jogos de {league === 'futsal' ? 'Futsal' : 'Campo'}
            </h3>
            <div className="space-y-3">
                  ...
                  <div key={match.id} className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="text-gray-900">{getTeamName(match.homeTeamId)}</div>
                      </div>
                      ...
                      <div className="flex-1 text-right">
                        <div className="text-gray-900">{getTeamName(match.awayTeamId)}</div>
                      </div>
                    </div>
              ...
              {matches.filter(m => m.league === league).length === 0 && (
                <p className="text-sm text-gray-500 text-center py-8">
                  Nenhum jogo de {league === 'futsal' ? 'futsal' : 'campo'} cadastrado
                </p>
              )}
            </div>
          </div>
