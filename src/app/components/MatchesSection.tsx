...
function TeamLogo({ logo, name, size = 'md' }: { logo?: string; name: string; size?: 'sm' | 'md' }) {
  ...
  return <div className={`${dim} bg-white/80 rounded-full flex items-center justify-center text-gray-600 flex-shrink-0`}>{name.charAt(0)}</div>;
}

export function MatchesSection({ activeLeague }: MatchesSectionProps) {
  const { matches: allMatches, teams, gameEvents } = useAdmin();
  ...
  const eventIcon = (type: string) => ({ goal: '⚽', yellow_card: '🟨', red_card: '🟥', assist: '🅰️' }[type] || '');
    ...
    <section>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          {(['all', 'finished', 'upcoming'] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`flex-1 sm:flex-none px-3 md:px-4 py-2 text-xs md:text-sm rounded-lg transition-colors ${filter === f ? 'bg-[#0f4c2e] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
      ...
      <div className="space-y-3 md:space-y-4">
        {matches.length > 0 ? matches.map((match) => {
          ...
          const matchEvents = gameEvents.filter(e => e.gameId === match.id).sort((a, b) => (a.minute ?? 999) - (b.minute ?? 999));
          const homeEvents = matchEvents.filter(e => e.teamId === match.homeTeamId);
          const awayEvents = matchEvents.filter(e => e.teamId === match.awayTeamId);
            ...
            <div key={match.id} className="bg-[#0f4c2e] rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="px-4 md:px-6 pt-4 pb-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div className="flex flex-col gap-1">
                  {match.competition && <span className="text-xs font-bold text-[#d4af37] uppercase tracking-wide">{match.competition}</span>}
                ...
                <span className={`px-2 md:px-3 py-1 text-xs rounded-full whitespace-nowrap font-medium ${match.status === 'finished' ? 'bg-green-200 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {match.status === 'finished' ? 'Finalizado' : 'Agendado'}
                </span>
              </div>

              {/* Corpo branco com times */}
              <div className="bg-white mx-3 mb-3 rounded-lg p-4 md:p-5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 text-right pr-2 md:pr-6 min-w-0">
                    ...
                    <div className="ml-auto w-fit"><TeamLogo logo={homeTeam?.logo} name={homeName} /></div>
                  </div>
                ...
                {matchEvents.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-gray-100">
                    <div className="grid grid-cols-2 gap-4">
                      {/* Eventos do time da casa */}
                      <div className="space-y-1">
                        {homeEvents.map(ev => (
                          <div key={ev.id} className="flex items-center gap-1.5 text-xs text-gray-600 justify-end">
          ...
          <div className="text-center py-12 text-gray-500"><p className="text-sm">Nenhum jogo cadastrado ainda</p></div>
        )}
      </div>
    </section>
  );
