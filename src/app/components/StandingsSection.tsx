...
export function StandingsSection({ activeLeague }: StandingsSectionProps) {
  ...
  // Ordenar times por pontos e saldo de gols
  const sortTeams = (teams: typeof teamStats) => {
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
        ...
        {teamStats.length === 0 ? (
          ...
          <div className="space-y-6">
            {groups.map(group => {
              const groupTeams = sortTeams(teamStats.filter(t => t.group === group));
              return (
                <div key={group}>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="px-3 py-1 bg-[#0f4c2e] text-white text-xs rounded font-medium">
                      Grupo {group}
                    </div>
                  </div>
            ...
            {teamStats.filter(t => !t.group).length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <div className="px-3 py-1 bg-gray-500 text-white text-xs rounded font-medium">
                    Sem Grupo
                  </div>
                </div>
                <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
                  <table className="w-full min-w-[280px]">
                    ...
                    <tbody>
                      {sortTeams(teamStats.filter(t => !t.group)).map((team, index) => (
                        <tr key={team.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="py-2">
                            <div className="w-5 h-5 flex items-center justify-center rounded text-xs text-gray-600">
                              {index + 1}
                            </div>
                          </td>
                          <td className="py-2">
          ...
          <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
            <table className="w-full min-w-[280px]">
              <thead>
                <tr className="border-b border-gray-200 text-left">
                  <th className="pb-2 md:pb-3 text-xs text-gray-600">#</th>
                  <th className="pb-2 md:pb-3 text-xs text-gray-600">Time</th>
                  <th className="pb-2 md:pb-3 text-xs text-gray-600 text-center">P</th>
                  <th className="pb-2 md:pb-3 text-xs text-gray-600 text-center">J</th>
                  <th className="pb-2 md:pb-3 text-xs text-gray-600 text-center">SG</th>
                </tr>
              </thead>
              <tbody>
                {sortTeams(teamStats).map((team, index) => (
                  <tr
                    key={team.id}
                    className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                  ...
                  >
                    <td className="py-2 md:py-3">
                      <div className={`w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded text-xs ${
                        index === 0 ? 'bg-[#0f4c2e] text-white' :
                        index < 4 ? 'bg-green-100 text-[#0f4c2e]' :
                        'text-gray-600'
                      }`}>
                        {index + 1}
                      </div>
                    ...
                    <td className="py-2 md:py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 md:w-6 md:h-6 bg-gray-200 rounded-full flex items-center justify-center text-xs flex-shrink-0">
                          {team.name.charAt(0)}
                        </div>
                        <span className="text-xs md:text-sm text-gray-900 truncate">{team.name}</span>
                      </div>
                    </td>
                    <td className="py-2 md:py-3 text-center text-sm md:text-base text-gray-900 font-medium">{team.points}</td>
                    <td className="py-2 md:py-3 text-center text-gray-600 text-xs md:text-sm">{team.played}</td>
                    <td className="py-2 md:py-3 text-center text-gray-600 text-xs md:text-sm">
                      {team.goalsFor - team.goalsAgainst > 0 ? '+' : ''}{team.goalsFor - team.goalsAgainst}
                    </td>
                  </tr>
                ))}
